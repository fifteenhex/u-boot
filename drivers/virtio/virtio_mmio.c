...
static inline u32 virtio_mmio_readl_raw(void *addr)
{
...
static inline u16 virtio_mmio_readw_raw(void *addr)
{
...
static inline void virtio_mmio_writel_raw(u32 val, void *addr)
{
...
{
	__raw_writew(cpu_to_le16(val), addr);
}

static inline void virtio_mmio_writew_raw(u32 val, void *addr)
{
				  ...
				  void *buf, unsigned int len)
{
	...
	case 1:
		...
		break;
	case 2:
		w = virtio_mmio_readw_raw(base + offset);
		memcpy(buf, &w, sizeof(w));
		break;
	case 4:
		l = virtio_mmio_readl_raw(base + offset);
		memcpy(buf, &l, sizeof(l));
		break;
	case 8:
		l = virtio_mmio_readl_raw(base + offset);
		memcpy(buf, &l, sizeof(l));
		l = virtio_mmio_readl_raw(base + offset + sizeof(l));
				  ...
				  const void *buf, unsigned int len)
{
	...
	case 2:
		...
		virtio_mmio_writew_raw(w, base + offset);
		break;
	case 4:
		memcpy(&l, buf, sizeof(l));
		virtio_mmio_writel_raw(l, base + offset);
		break;
	case 8:
		memcpy(&l, buf, sizeof(l));
		virtio_mmio_writel_raw(l, base + offset);
		memcpy(&l, buf + sizeof(l), sizeof(l));
		virtio_mmio_writel_raw(l, base + offset + sizeof(l));
		break;
	default:
		WARN_ON(true);
	}
...
static int virtio_mmio_probe(struct udevice *udev)
{
	...
	uc_priv->vendor = virtio_mmio_readl_raw(priv->base + VIRTIO_MMIO_VENDOR_ID);
